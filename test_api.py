...
ok(code == 200 and any(n["id"] == novel_id for n in novels), f"列出小说 -> {len(novels)} 部")
...
# 3. Outline CRUD (Volume → Chapter → Scene)
print("\n[3] 大纲节点 CRUD")
...
code, ch = req("POST", "/outlines", {
    "novel_id": novel_id, "parent_id": vol_id,
    "node_type": "chapter", "title": "第一章：启航",
    "summary": "主角开启旅程", "sort_order": 0
})
ok(code == 200 and ch.get("node_type") == "chapter", f"创建章 -> {ch.get('title')} (id={ch.get('id')})")
ch_id = ch["id"]

code, sc = req("POST", "/outlines", {
    "novel_id": novel_id, "parent_id": ch_id,
    "node_type": "scene", "title": "第一节：出发",
    "summary": "主角决定离开地球", "sort_order": 0
})
ok(code == 200 and sc.get("node_type") == "scene", f"创建节 -> {sc.get('title')} (id={sc.get('id')})")
sc_id = sc["id"]

code, outline = req("GET", f"/outlines/novel/{novel_id}")
ok(code == 200 and len(outline) == 1,
   f"大纲树 -> {len(outline)} 卷")
vol_children = outline[0].get("children", [])
ok(len(vol_children) == 1 and len(vol_children[0].get("children", [])) == 1,
   f"子节点 -> 章={len(vol_children)}, 节={len(vol_children[0].get('children', []))}")
...
code, novels = req("GET", "/novels")
ok(code == 200 and not any(n["id"] == novel_id for n in novels), f"确认清空 -> 测试小说已删除")
